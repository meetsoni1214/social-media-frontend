...
const gradientIconButtonVariants = cva(
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        primary:
...
>(({ className, variant, size, ...props }, ref) => {
  return (
    <button
      className={cn(gradientIconButtonVariants({ variant, size, className }))}
      ref={ref}
      {...props}
    />
  );
